...
def get_new_tweets(screen_name, last_id=None):
    ...
    outtweets = [(tweet.id_str, screen_name, tweet.created_at,
                  tweet.text, tweet.favorite_count, tweet.retweet_count) for tweet in new_tweets]
...
if __name__ == '__main__':
    if not os.path.exists(TWEETS_PATH):
        with open(TWEETS_PATH, "wt") as f:
            writer = csv.writer(f)
            writer.writerow(["id", "user_id", "created_at", "text", "retweets", "favorites"])
    ...
    with open(LAST_IDS_JSON, "wt") as f:
